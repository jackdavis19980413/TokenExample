...
const Fund = ({ isOpen, project, onCloseFunc, refresh, refreshAccount }) => {
    ...
    const checkEtherBalance = async () => {
        ...
        const balanceWei = await web3Instance.eth.getBalance(owner);
    ...
    const onFund = async () => {
        try {
            ...
            await contract.methods.fundProject(project.id).send({ from: owner, value: weiAmount, gas: gasLimit });
            // checkEtherBalance()
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-filter backdrop-blur-sm">
            <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-96 relative">
                ...
                <h2 className="text-gray-200 text-2xl font-bold mb-6">Fund the {project.name}</h2>
                <div className="mb-6">
                    <InputLabel
                        className='py-2'
                        htmlFor="get_balance"
