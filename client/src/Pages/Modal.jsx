...
const Modal = ({ isOpen, price, onCloseFunc }) => {
    ...
    useEffect(() => {
        const initWeb3 = async () => {
            try {
                ...
                const weiAmount = Web3.utils.toWei(ethBalance, 'ether');
                setAmount((await contract.methods.getTokenAmount(weiAmount).call()).toString())
    ...
    const onBuyToken = async () => {
        try {
            ...
            const gasLimit = 300000; // Adding a buffer to the gas estimate
            const receipt = await contract.methods.buyTokens().send({ from: userAccount, value: weiAmount, gas: gasLimit });
            checkEtherBalance()
            refreshBalance()
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-filter backdrop-blur-sm">
            <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-96 relative">
                ...
                <h2 className="text-gray-200 text-2xl font-bold mb-6">Buy GREEN Token</h2>
                <div className="mb-6">
                    <InputLabel
                        className='py-2'
                        htmlFor="amount_green_token"
                        value="Amount Green Token">
                    </InputLabel>
                    <div className="relative w-full">
                        <TextInput
                            id="amount_green_token"
                            type="text"
                            disabled
                            value={amount}
                    ...
                    <span className="text-blue-500 mt-1 block">Balance: {Web3.utils.fromWei(balance, 'ether')} GET</span>
                </div>
                <div className="mb-6">
                    <InputLabel
                        className='py-2'
